...
async function hapusData(id){
  ...
  try { await deleteDoc(doc(db,"pengeluaran",id)); loadData(); }
...
function updateChart(totalMasuk,totalKeluar){
  ...
  chartInstance=new Chart(ctx,{
    type:"bar",
    data:{ labels:["Pemasukan","Pengeluaran"], datasets:[{label:"Jumlah (Rp)", data:[totalMasuk,totalKeluar], backgroundColor:["#27ae60","#e74c3c"]}] },
    options:{ responsive:true, plugins:{legend:{display:false}, title:{display:true,text:"Perbandingan Pemasukan vs Pengeluaran (Bulan Dipilih)"}}, scales:{y:{beginAtZero:true}} }
  });
}
...
function exportToCSV(){
  ...
  const rows=allData.slice().sort((a,b)=>a.tanggal-b.tanggal).map(d=>`${d.tanggal.toISOString().split("T")[0]},${d.jenis},${d.nominal},"${d.keterangan}"`);
  csv+=rows.join("\n");
